...
```mermaid
flowchart TD
    H[人类用户] --> C[Codex / Agent]: 用户可给AI Agent输入自然语言指令
    H --> O[Operator Console]: 用户可通过网页端Operator Console进行配置、查询、管理敏感信息
    C -->|MCP 调用| M[MCP Server]: AI Agent 通过 MCP 调用钱包能力
    ...
    subgraph Local["本地环境"]
        ...
        O --> OS[Operator Service]
        OS --> S
        OS --> ENV[.env 配置]

        S --> PROPOSALS[proposals.json]
        S --> STATE[wallet_state.json]
        S --> BOOK[address_book.json]
        S --> WHITE[whitelist.json]
        S --> FUNDING[funding_events.jsonl]
        S --> AUDIT[audit.jsonl]
